...
public class ProductController {
  ...
  @GetMapping("/add/{productName}/{productUrl}/{productMoney}/{productDescription}/{productState}"
      +
      "/{productTime}/{Seller}")
  public String addProducts(@PathVariable("productName") String productName,
                            @PathVariable("productUrl") String productUrl,
                            @PathVariable("productMoney") int productMoney,
                            @PathVariable("productDescription") String productDescription,
                            @PathVariable("productState") int productState,
                            @PathVariable("productTime")  int productTime,
                            @PathVariable("Seller") int seller) {
    try {
      String encodedText = new String(decoder.decode(productUrl), "UTF-8");
      return productManager.addProducts(productName,
          encodedText, productMoney, productDescription, productState, productTime, seller);
    } catch (UnsupportedEncodingException e) {
      return productManager.addProducts(productName, productUrl, productMoney,
          productDescription, productState, productTime, seller);
    }
  }

  @GetMapping("update/{ID}/{productName}/{productUrl}/{productMoney}/{productDescription}")
  public List<Product> updateProducts(@PathVariable("ID") int id,
                                   @PathVariable("productName") String productName,
                                   @PathVariable("productUrl") String productUrl,
                                   @PathVariable("productMoney") String productMoney,
                                   @PathVariable("productDescription") String productDescription) {
